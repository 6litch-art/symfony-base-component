...
class Box implements ImageBoxInterface
{
    ...
    public function __construct(int $width, int $height, int $type = self::TYPE_ABSOLUTE)
    {
        ...
        if (self::TYPE_NONE === $type) {
            $this->box = new ImageBox(300, 150);
        } else {
            $this->box = new ImageBox($width, $height);
        }
    }
    ...
    /**
     * @return int one of the SvgImageBox::TYPE_* constants
     */
    ...
    public function contains(ImageBoxInterface $box, PointInterface $start = null): bool
    {
