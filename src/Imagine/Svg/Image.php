...
class Image extends AbstractImage
{
    ...
    public function crop(PointInterface $start, BoxInterface $size): self
    {
        ...
        if (
            ...
            $this->resize(Box::createTypeAbsolute($currentSize->getWidth(), $currentSize->getHeight()));
        }
    ...
    public function thumbnail(BoxInterface $size, $settings = ImageInterface::THUMBNAIL_INSET, $filter = ImageInterface::FILTER_UNDEFINED): self
    {
        ...
        if (Box::TYPE_ABSOLUTE === $newSizeType && Box::TYPE_ABSOLUTE === $this->getSize()->getType()) {
        ...
        if (
            Box::TYPE_ASPECT_RATIO === $newSizeType
            && ($settings & ImageInterface::THUMBNAIL_OUTBOUND)
        ) {
            $thumb->thumbnail(
                Box::createTypeAbsolute($size->getWidth(), $size->getHeight()),
                $settings,
                $filter
            );
        ...
        if (
            Box::TYPE_ABSOLUTE === $newSizeType
            && Box::TYPE_ASPECT_RATIO === $thumb->getSize()->getType()
        ) {
            $thumb->resize(
                Box::createTypeAbsolute($thumb->getSize()->getWidth(), $thumb->getSize()->getHeight()),
                $filter
            );
        ...
        if (Box::TYPE_ASPECT_RATIO === $newSizeType) {
            return $thumb->resize(
                Box::createTypeAspectRatio($thumb->getSize()->getWidth(), $thumb->getSize()->getHeight()),
                $filter
            );
        }
    ...
    public function getSize(): Box
    {
        ...
        // Absolute dimensions
        if ($width && $height) {
            return Box::createTypeAbsolute($width, $height);
        }
        ...
        return Box::createTypeAspectRatio($viewBoxWidth, $viewBoxHeight);
    }
