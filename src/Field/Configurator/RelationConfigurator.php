...
final class RelationConfigurator implements FieldConfiguratorInterface
{
    ...
    {
        return RelationField::class === $field->getFieldFqcn();
    }
    ...
    public function configure(FieldDto $field, EntityDto $entityDto, AdminContext $context): void
    {
        ...
        $targetCrudControllerFqcn = $field->getCustomOption(RelationField::OPTION_CRUD_CONTROLLER)
            ?? $context->getCrudControllers()->findCrudFqcnByEntityFqcn($targetEntityFqcn);
        $field->setCustomOption(RelationField::OPTION_CRUD_CONTROLLER, $targetCrudControllerFqcn);

        if (RelationField::WIDGET_AUTOCOMPLETE === $field->getCustomOption(RelationField::OPTION_WIDGET)) {
        ...
        if (true === $field->getCustomOption(RelationField::OPTION_AUTOCOMPLETE)) {
            $targetCrudControllerFqcn = $field->getCustomOption(RelationField::OPTION_CRUD_CONTROLLER);
            ...
            $autocompleteEndpointUrl = $this->adminUrlGenerator
                ...
                ->setController($field->getCustomOption(RelationField::OPTION_CRUD_CONTROLLER))
                ->setAction('autocomplete')
                ->setEntityId(null)
                ->unset(EA::SORT) // Avoid passing the 'sort' param from the current entity to the autocompleted one
                ->set(RelationField::PARAM_AUTOCOMPLETE_CONTEXT, [
            ...
            $field->setFormTypeOptionIfNotSet('query_builder', static function (EntityRepository $repository) use ($field) {
                ...
                if ($queryBuilderCallable = $field->getCustomOption(RelationField::OPTION_QUERY_BUILDER_CALLABLE)) {
    ...
    {
        $field->setCustomOption(RelationField::OPTION_DOCTRINE_ASSOCIATION_TYPE, 'toOne');
