    ...
    public function highlightHeadings(mixed $json, array $attrs = [], ?int $maxLevel = null): mixed
    {
        ...
        foreach($json->blocks ?? [] as $block) {
            ...
            if($block->type == "header" && $block->data->level < $maxLevel) {
                ...
                $id = strtolower($this->slugger->slug(html_entity_decode(strip_tags($text))));
    ...
    public function getTableOfContents(mixed $json, ?int $maxLevel = null): array
    {
        ...
        foreach($json->blocks ?? [] as $block) {
            ...
            if($block->type == "header" && $block->data->level < $maxLevel) {
                ...
                $text = $block->data->text;
                $id = str_strip_specials(strtolower($this->slugger->slug($text)));

                $headlines[] = [
                        "tag" => "h".$block->data->level,
                        "slug"  => $id,
                        "title" => html_entity_decode(strip_tags($text))
                ];
            }
        }
