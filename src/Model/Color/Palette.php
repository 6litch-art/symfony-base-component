...
class Palette implements \Countable, \IteratorAggregate
{
    ...
    public function count(): int { return count($this->colors); }
    public function getIterator(): ArrayIterator { return new \ArrayIterator($this->colors); }

    public function getCount(int $color): int { return $this->colors[$color]; }
    public function getDominantColors(int $limit = null): array { return array_slice($this->colors, 0, $limit, true); }
    ...
    {
        if($filenameOrImage instanceof GdImage) $image = $filenameOrImage;
