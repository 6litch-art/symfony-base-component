...
use Base\Admin\Config\WidgetItem;
...
use Base\Admin\Config\MenuAfterItem;
use Base\Controller\Admin\Crud\Layout\Widget\SlotCrudController;
...
class AbstractDashboardController extends \EasyCorp\Bundle\EasyAdminBundle\Controller\AbstractDashboardController
{
    ...
    public function Manual(?string $slug = null): Response
    {
        ...
        if($slug !== null && $slug == first($documents)?->getSlug()) {
            return $this->redirectToRoute("admin_manual");
        }

        return $this->render('@Wikidoc/admin/manual.html.twig', [
    ...
    public function ApiKey(Request $request, array $fields = []): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $notification = new Notification("@controllers.admin_apikey.success");
            $notification->send("success");

            return $this->router->reloadRequest();
        }

        return $this->render('admin/apikey.html.twig', [
    ...
    public function Settings(Request $request, array $fields = []): Response
    {
        $fields = array_reverse(array_merge(array_reverse([
            ...
            "base.settings.logo.admin" => ["form_type" => ImageType::class, "multiple" => false, "required" => false],
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $notification = new Notification("@controllers.admin_settings.success");
            $notification->send("success");
    ...
    public function Widgets(Request $request, array $widgetSlots = []): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $notification = new Notification("@controllers.admin_widgets.success");
            $notification->send("success");
        ...
        return $this->render('admin/widgets.html.twig', ["form" => $form->createView()]);
    }
    ...
    public function configureDashboard(): Dashboard
    {
        ...
        $title = $this->settingBag->getScalar("base.settings.title") ?? $this->translator->trans("admin.title", [], self::TRANSLATION_DASHBOARD);
        $subtitle = $this->settingBag->getScalar("base.settings.slogan") ?? $this->translator->trans("admin.subtitle", [], self::TRANSLATION_DASHBOARD);
    ...
    {
        $menu = [];
        $menu[] = MenuItem::linkToRoute("admin_apikey");
        $menu[] = MenuItem::linkToRoute("admin_settings");
        $menu[] = MenuItem::linkToRoute("admin_widgets");
