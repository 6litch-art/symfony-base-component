...
${reason}`);
  ...
  // example/client.js
  var WS_URL = window.COLLAB_DEMO_WS_URL || "ws://localhost:1234";
  var ROOM = "demo\\Room:1:content:_";
  var DEV_SECRET = window.COLLAB_DEMO_SECRET || "dev-secret";
  ...
  async function mintDevTicket(room, user) {
    const payload = { uid: user.id, name: user.name, avatar: null, color: user.color, room, exp: Math.floor(Date.now() / 1e3) + 300 };
  ...
  async function createPane(paneEl, user) {
    ...
    sendEl.addEventListener("click", () => {
      ...
      if (!text2) return;
      yarray.push([{ author: user.name, color: user.color, text: text2 }]);
      inputEl.value = "";
    });
